...
module Tool {
     ...
     * @returns (description)
     */
    export function registForwardingRouter
        (router: any, urlItem: kdm.NodeUrlItem<any, any>, bindUrl: string, extendRequestObj: any = {}) {

        var type;

        //todo: 这里应该用urlMethodType枚举的引用，而不是直接写死0 1 2

        if (urlItem.type == 0) {//kdm.urlMethodType.PAGE
            return;
        } else if (urlItem.type == 1) {//kdm.urlMethodType.GET
            type = "get";
        } else if (urlItem.type == 2) {//kdm.urlMethodType.POST
            type = "post";
        } else {
